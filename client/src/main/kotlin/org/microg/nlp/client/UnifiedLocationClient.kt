...
class UnifiedLocationClient(private val context: Context, private val lifecycle: Lifecycle) : LifecycleOwner {
    ...
    private fun onServiceConnected(name: ComponentName, binder: IBinder) {
        ...
        lifecycleScope.launchWhenStarted {
            ...
            context?.resources?.getStringArray(R.array.force_location_backends)?.let { setLocationBackends(it + getLocationBackends()) }
            context?.resources?.getStringArray(R.array.force_geocoder_backends)?.let { setGeocoderBackends(it + getGeocoderBackends()) }
            updateServiceInterval()
